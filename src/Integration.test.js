...
test('End2End - Gameboard fetch load grid from backend and find path of cost 44', async () => {
  render(<Gameboard />);
  ...
  await waitFor(() => expect(screen.getByRole('status')).toHaveTextContent('Costo del cammino minimo: 44'),
      {
        timeout: TIMEOUT,
        interval: 10000,
        onTimeout: (error) => {
